...
namespace MSGer.tk
{
    public partial class MainForm : Form
    {
        ...
        //public static ObjectListView contactList2;
        public MainForm()
        {
            InitializeComponent();
            Thread.CurrentThread.Name = "Main Thread";
            //contactList.Columns[0].Width = contactList.ItemHeight; //2014.02.28.
            //contactList2.Columns[0].Width = contactList2.RowHeight; //2014.04.30.
            //^^ Szerintem egyébként felesleges, majd ha átállitom, átállitom ott is
            //var tmp = new List<string>();
            //tmp.Add("Betöltés...");
            //contactList2.SetObjects(tmp);
            ...
            try
            {
                ...
                contactList.Items[0].SubItems[0].Text = Language.GetCuurentLanguage().Strings["loading"];
        ...
        //public List<Control[]> ContactList;
        public void UpdatePartnerList()
        {
            while (PartnerListThreadActive && MainThread.IsAlive)
            {
                ...
                //List<RichListViewItem> listViewItems = new List<RichListViewItem>();
                //ContactList = new List<Control[]>();
                ...
                if (!this.IsDisposed)
                {
                    this.Invoke((MethodInvoker)delegate
                    {
                        ...
                        //new ThreadSetVar(contactList, "clear", this, Program.ThreadTarget);
                        int i = 0;
                        ...
                        { //-1: 2014.04.04. - A végére is odarak egy elválasztó jelet, ami miatt eggyel több elem lesz a tömbben
                            ...
                            //contactList.Items.Add(new RichListViewItem());
                            var oneitem = new Control[2];
                            //oneitem.Add(new RichListViewItem(2));
                            PictureBox item = new PictureBox();
                            if (tmp.Count <= 1) //2014.04.28.
                                tmp.Add(0);
                            ...
                            //listViewItems[i].SubItems[0].Control = item;
                            oneitem[0] = item;
                            //listViewItem[i].SubItems[1].Text = row[x] + " " + state + "\n" + row[x + 1];
                            //ExRichTextBox item2 = new ExRichTextBox(); //2014.04.28.
                            //item2.AppendText(row[x] + " " + state + "\n" + row[x + 1]);
                            ExRichTextBox item2 = new ExRichTextBox();
                            //item2.Rtf = TextFormat.Parse(row[x] + " " + state + "\n" + row[x + 1]);
                            //item2 = TextFormat.Parse(item2, row[x] + " " + state + "\n" + row[x + 1]);
                            item2.Text = row[x] + " " + state + "\n" + row[x + 1];
                            item2 = TextFormat.Parse(item2);
                            //listViewItems[i].SubItems[1].Control = item2;
                            oneitem[1] = item2;
                            //new ThreadSetVar(contactList, oneitem, this, Program.ThreadTarget);
                            //ContactList.Add(oneitem);
                            /*var tmp2 = new RichListViewItem(contactList.Columns.Length);
                            tmp2.SubItems = oneitem;*/
                            //new ThreadSetVar(contactList.Items, oneitem, ThreadSetVar.UpdateMode_AddRichListViewItem);
                            items.Add(oneitem);
                            i++;
                        }
                        Settings.Default.picupdatetime = String.Join(",", tmp.Select(ix => ix.ToString()).ToArray());
                        UserInfo.Partners = tmpuser;
                        //new ThreadSetVar(contactList.Items, items, this);
                        //contactList.Items.Clear();
                        //Action<RichListView, List<Control[]>> contactList_SetItems = list => contactList.SetItems();
                        //contactList_SetItems(items);
                        //this.Invoke((MethodInvoker)delegate
                        //{
                        contactList.Items.Clear();
                        ...
                        /*try
                        {
                            //contactList.Items.AddRange(listViewItems.ToArray()); //2014.03.01. - ToArray: 2014.03.21.
                            contactList.Items.(listViewItems.ToArray());
                        }
